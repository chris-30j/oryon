...
export default function Contact() {
  ...
  return (
    <div className="container" style={{ padding: '4rem 1.5rem', maxWidth: '800px' }}>
      ...
      <p style={{ marginBottom: '2rem', fontSize: '1.1rem' }}>Have questions about our robotics kits or training programs? We&apos;d love to hear from you!</p>
      ...
      <div style={{ background: 'white', padding: '3rem', borderRadius: '12px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)' }}>
        <form id="contact-form" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          {status && <p style={{ marginTop: '1rem', fontWeight: 'bold', color: status.includes('successfully') ? 'green' : 'var(--primary-color)' }}>{status}</p>}
        </form>
      </div>
      ...
      <div style={{ marginTop: '3rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
        <div style={{ background: 'white', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
          ...
          <p>info@oryonrobotics.com</p>
        </div>
        <div style={{ background: 'white', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
          <h3 style={{ color: 'var(--primary-color)', marginBottom: '0.5rem' }}>📞 Phone</h3>
          <p>+91 9150084761</p>
        </div>
        <div style={{ background: 'white', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
          <h3 style={{ color: 'var(--primary-color)', marginBottom: '0.5rem' }}>📍 Address</h3>
          <p>Chennai, India - 600040</p>
        </div>
      </div>
    </div>
  );
